...
dotnet build ./src/NEventStore.Persistence.MongoDB.Core.2017.sln -c $configuration --no-restore

# Testing
if ($runtests -eq "y") {
    Write-Host "Executing Tests"
    dotnet test ./src/NEventStore.Persistence.MongoDB.Core.2017.sln -c $configuration --no-build
